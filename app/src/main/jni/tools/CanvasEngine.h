...
public:
    ...
    void DrawCircle(Color color, Vector2 c, float radius) {
        if (isValid()) {
            jclass canvasView = _env->GetObjectClass(_view);
            jmethodID drawCircle = _env->GetMethodID(canvasView, "DrawFilledCircle",
	...
	void DrawCurveRect(Color color, float thickness, Vector2 start, Vector2 end) {
        if (isValid()) {
			jclass canvasView = _env->GetObjectClass(_view);
            jmethodID drawCircle = _env->GetMethodID(canvasView, "DrawCurvedRect",
                                                             "(Landroid/graphics/Canvas;IIIIFFFFF)V");													 
			              
				_env->CallVoidMethod(_view, drawCircle, _canvas, (int) color.a, (int) color.r,
                                 (int) color.g, (int) color.b, thickness, start.X, start.Y, end.X, end.Y);			 
    ...
    void DrawFilledCurveRect(Color color, float thickness, Vector2 start, Vector2 end) {
        if (isValid()) {
	        ...
	        jclass canvasView = _env->GetObjectClass(_view);
            jmethodID drawCircle = _env->GetMethodID(canvasView, "DrawFilledCurvedRect",
                                                             "(Landroid/graphics/Canvas;IIIIFFFFF)V");											 
