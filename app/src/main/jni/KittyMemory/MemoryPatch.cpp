...
//  Modified by Ali (@M3xecv) on 23/01/2023
//
...
MemoryPatch::MemoryPatch(uintptr_t absolute_address, 
  ...
  MemoryPatch MemoryPatch::createWithFloat(uintptr_t absolute_address, float floatValue) {
    MemoryPatch patch;
    ...
    patch._address = absolute_address;
    patch._size = sizeof(float);

    patch._orig_code.resize(patch._size);
    patch._patch_code.resize(patch._size);

    // Convert float value to binary representation
    std::memcpy(&patch._patch_code[0], &floatValue, sizeof(float));
...
MemoryPatch MemoryPatch::createWithInt(uintptr_t absolute_address, int intValue) {
    MemoryPatch patch;
    ...
    patch._address = absolute_address;
    patch._size = sizeof(int);

    patch._orig_code.resize(patch._size);
    patch._patch_code.resize(patch._size);

    // Convert int value to binary representation
    std::memcpy(&patch._patch_code[0], &intValue, sizeof(int));
